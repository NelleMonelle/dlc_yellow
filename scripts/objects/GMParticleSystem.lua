	...
	if part then
		...
		if part_type.grav ~= 0 or self.acount > 0 then
			...
			if part_type.grav ~= 0 then
				local h2 = part_type.grav * math.cos(part.grav_dir)
				local v2 = part_type.grav * math.sin(part.grav_dir)
				xspd = xspd + h2 * DTMULT
				yspd = yspd + v2 * DTMULT
			end
...
function GMParticleSystem:handleShape(part_type, part)
	if part then
		...
		if part_type.sprite_anim and part_type.frame_spd > 0 then
...
function GMParticleSystem:partTypeSprite(part_type, sprite, anim, stretch, rand)
	if part_type then
...
function GMParticleSystem:draw()
	super.draw(self)
	love.graphics.push()
	for i, emitter in ipairs(self.emitters) do
		for _, part in ipairs(emitter.particles) do
