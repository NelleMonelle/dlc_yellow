local actor, super = Class(Actor, "jandroidenemy_hurt")
...
function actor:init()
    ...
    self.animations = {
        ["lightbattle_hurt"] = {"hurt", 1/10, false},
    ...
    self:addLightBattlerPart("body_top", {
        ["create_sprite"] = function()
            local sprite = Sprite(self.path.."/body_top_hurt", 30, 68)
            sprite:setOrigin(0.5, 1)
            --sprite.layer = 400
            return sprite
        end
    ...
    self:addLightBattlerPart("broom", {
        ["create_sprite"] = function()
            local sprite = Sprite(self.path.."/broom_hurt", 65, 100)
            sprite:setOrigin(0.5, 1)
            --sprite.layer = 400
            return sprite
        end
